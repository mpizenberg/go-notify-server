...
- `tag` — string identifier that groups notifications, useful for updating rather than flooding. It works on two levels: (1) it is sent in the payload so the browser **replaces** an already-displayed notification carrying the same tag (Android/desktop); (2) the server additionally derives an RFC 8030 `Topic` header from it (a hash of the tag), which makes the push service **collapse** an earlier, still-undelivered message with the same tag. On Apple's gateway this `Topic` maps to `apns-collapse-id` — the mechanism that actually makes **iOS** replace notifications, since iOS ignores the payload `tag` for replacement and would otherwise stack them. Note that the collapse can drop the earlier message entirely if it is still pending when the newer one arrives (latest-wins), so reuse a tag only for notifications where superseding the previous one is intended. This `tag` is unrelated to the routing `topic` above.
