...
- `tag` — string identifier that groups notifications, useful for updating rather than flooding. It works on two levels:
  1. **Displayed-notification merge.** The tag is sent in the payload so the user agent **replaces** an already-shown notification carrying the same tag instead of stacking a new one. This works on Android/desktop Chrome/Firefox.
  2. **Queued-message de-dup.** The server additionally derives an RFC 8030 `Topic` header from the tag (a `blake2b` hash, since the header is limited to 32 URL-safe characters). The push service keeps at most one *still-undelivered* message per Topic, so a burst of same-tag notifications sent while a device is **offline** collapses to just the latest on reconnect (latest-wins). This does **not** affect notifications that have already been delivered and shown.

  **Safari / iOS limitation:** the displayed-notification merge (level 1) is currently **broken on Apple platforms** — Safari and iOS ignore `tag` for replacement and stack the notifications anyway ([WebKit bug 258922](https://bugs.webkit.org/show_bug.cgi?id=258922)). The `Topic` header (level 2) still works for the offline-backlog case, but while a device is online each push is delivered immediately, so same-tag notifications received in quick succession will still appear as separate notifications on Safari/iOS. There is no server-side fix for this; the only mitigation is to debounce/rate-limit same-tag sends so the bursts are not produced in the first place.

  This `tag` is unrelated to the routing `topic` above.
...
- `data` — arbitrary JSON object passed through as `notification.data` in the push payload. Commonly used to carry a `url` field that the service worker reads in its `notificationclick` handler (e.g. `"data": {"url": "/messages/123"}`), but any key/value pairs are accepted. In declarative mode, `data.url` also seeds the `navigate` field (see below).
- `navigate` (derived) — the destination URL Safari opens when a declarative notification is clicked, which the spec **requires**. You do not set it directly: in declarative mode the server fills `notification.navigate` from `data.url`, defaulting to `/` when absent. The same URL stays in `data.url` so the service-worker `notificationclick` path (other browsers, or `legacy` mode) keeps working. Not added in `legacy` mode, where the service worker reads `data.url` instead.
- `mutable` — if present, sets the top-level `"mutable"` flag on the declarative envelope, which makes Safari **also** fire the service worker `push` event so it can localize/transform the notification or run custom click logic; if the worker shows a replacement notification in time it is used, otherwise the declarative payload is shown as fallback. Omit this field entirely (the default) to let Safari render natively without waking the worker. Has no effect in `legacy` mode (the worker always runs there). Only meaningful if your service worker actually handles the push event (e.g. defines `SW_TRANSFORM_NOTIFICATION`).
